...
const isPricingField = (item) => item.kind === 'pricing' || /报价|价格|单价|总价|金额|税额|税率|合价|折扣|下浮|费率|费用|price|amount|cost/i.test(`${item.title} ${item.section || ''}`);
...
function normalizeAnalysis(payload, source) {
  ...
  for (const group of GROUPS) {
    ...
    result[group] = (payload[group] || []).map((item) => {
      const title = clean(item.title);
      const quote = clean(item.quote);
      if (!title || !quote || !compact(source.text).includes(compact(quote))) throw new Error('商务分析缺少可核对的原文摘录，请检查解析原文后重新提取');
...
function importEvidence(snapshot, root, asOf) {
  if (!Array.isArray(snapshot?.records)) throw new Error('请选择公司证据库 records 快照 JSON');
  const seen = new Set();
  let excluded = 0;
  const items = [];
  for (const row of snapshot.records) {
    if (row.companyId && row.companyId !== COMPANY) { excluded += 1; continue; }
    ...
    seen.add(id);
    const reasons = [];
    if (row.companyId !== COMPANY || row.verified !== true) reasons.push('公司归属待核实');
    ...
    if (row.kind === 'certificate') {
      ...
      if (employment?.companyId !== COMPANY || employment?.verified !== true || !employmentCheck.valid) reasons.push('人员任职归属证据待核实');
...
function buildDraft(state) {
  ...
  const selected = (state.evidence || []).filter(item => item.eligible && item.confirmed);
  const pending = [];
  ...
  add('政策声明', `投标人：${COMPANY}\n\n中小企业、监狱企业、残疾人福利性单位、信用及其他政策身份：【待核实】\n\n声明正文及适用的政策条款：【待核实：按招标原始表单填写，核验身份依据后由授权人确认】\n\n不因材料缺失自动声明符合政策条件。\n\n签字及盖章：【待人工完成】`);
  add('招标表单字段', table(['所属表单', '字段', '填写内容', '原文依据'], a.fields.map(item => [item.section || '待核实', item.title, isPricingField(item) ? '【待人工填写：报价】' : value(item.id, item.title), provenance(item)])));
