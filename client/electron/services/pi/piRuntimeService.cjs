...
function createPiRuntimeService({ app, configStore, aiService, isMonitorActive, onMonitorEvent, requestUserQuestion }) {
    ...
    startPromise = (async () => {
      ...
      proxy = createAgentOpenAiProxy({
        ...
        getActivityContext: () => activeTask ? {
          ...
          text_model_selection: activeTask.text_model_selection,
        } : null,
        verifyLoopback: true,
  ...
  async function runTask(payload = {}) {
    ...
    activeTask = {
      ...
      text_model_selection: payload.text_model_selection || payload.textModelSelection,
  ...
  async function runSelfCheck() {
    ...
    try {
      ...
      if (Object.keys(sessionSnapshot).length) {
        setStep('resources', snapshotValidation.resourcesValid ? 'success' : 'error', snapshotValidation.resourcesValid ? '仅加载联智标内置工作区指令' : 'Pi 资源加载结果不符合配置');
      } else {
