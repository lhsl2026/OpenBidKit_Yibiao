...
function emptyState() {
  return { companyName: COMPANY, projectName: '', deadline: '', files: [], evidence: [], excludedEvidence: 0,
    analysis: null, analysisComplete: false, analysisConfirmed: false, analysisTask: null, analysisCoverage: null,
    fieldValues: {}, textModelSelection: null, draft: null };
}
...
function createBusinessBidStore({ app, db, fileService }) {
  ...
  const updateBusinessBidWithoutReload = db.transaction((patch) => {
    const next = { ...loadBusinessBid(), ...patch, companyName: COMPANY };
  ...
  return {
    ...
    importEvidenceFile(filePath) {
      ...
      const result = importEvidence(snapshot, path.dirname(filePath), asOf(loadBusinessBid()));
    ...
    saveReview(payload) {
      ...
      if (payload.fieldValues) {
        patch.fieldValues = { ...state.fieldValues, ...payload.fieldValues };
        for (const item of state.analysis?.fields || []) if (isPricingField(item)) delete patch.fieldValues[item.id];
      }
      if (payload.evidence) {
    ...
    getExportPayload(kind) {
      ...
      return { project_name: `${state.projectName || COMPANY}_${kind === 'pending' ? '待补资料清单' : '商务标草稿'}`, table_pagination: 'keep-rows', outline: kind === 'pending' ? [checklist] : [...state.draft.sections, checklist] };
    },
    clear() { assertIdle(); return save({ ...emptyState(), textModelSelection: loadBusinessBid().textModelSelection }); },
  };
}
