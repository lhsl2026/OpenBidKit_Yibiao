...
- [ ] 以已核验/未核验业绩、人员证书和异名主体编写失败测试。
- [ ] 实现确定性公司画像、覆盖统计和哈希版本。
- [ ] 实现带幂等状态的预读画像导入，失败时 readiness 保守降级。
- [ ] 在本机配置启用同步，确认预读数据库只保留目标主体并回读统计。
- [ ] 提交公司证据画像改动。
...
- [ ] 编写预算带单位括号、付款、投标/履约保证金、评分未闭合的失败测试。
- [ ] 从 `knowledgeContent` 生成带证据状态的决策字段，并绑定精确报告版本。
- [ ] 让卡片和归档文档优先使用决策字段，缺失保持待核实。
- [ ] 修复预读预算格式兼容，导出补丁、运行 Jest 与构建。
- [ ] 重建预读镜像并回读两个真实项目的关键字段。
- [ ] 提交商务字段改动。
