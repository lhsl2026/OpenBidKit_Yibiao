...
- [x] 用真实招标文件和当前 handoff 建立隔离验收任务。
- [x] 逐阶段保存目录、事实和失败小节决策确认，未知事实保留占位。
- [x] 生成并检查 DOCX 结构、标题、章节和验收标识。
- [x] 投递测试群并回读文件消息；记录哈希与消息收据。
- [x] 提交真实写标验收工具和记录。
...
- [x] 编写生产群白名单、显式切流标志和测试/生产隔离的失败测试。
- [x] 实现生产启动检查与只读诊断命令。
- [x] 核对正式目标群和常开执行账户条件。
- [x] 安装或更新监管任务并验证重启恢复。
- [x] 在正式群发送单条上线验证卡并回读；失败则恢复测试配置。
- [x] 运行完整验证、更新验收记录并提交。
