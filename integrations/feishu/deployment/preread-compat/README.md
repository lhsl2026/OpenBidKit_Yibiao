...
Body: `{ "documentId": "<existing document UUID>" }`. It uses the existing relay authorization guard and validated DTO; extra properties are rejected. It calls the original `PrereadParserService.retryFactExtraction(taskId, documentId)` inline path, including its persisted-text loading, retry state guards, report preparation and atomic finalization. It returns `{ extraction, report? }`. It does not create a deferred extraction job, upload a file, or send messages.
